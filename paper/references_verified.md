...
<!-- 검증 원칙: 이 목록의 모든 항목은 (a) 이 세션에서 arXiv 초록 페이지/검색으로
     1차 확인했거나 (b) 저장소의 검증된 문헌 파일(research_review_prior_work.md,
     commit a94e9f0)에 링크와 함께 수록된 항목이다. 저자 전체 명단을 이 세션에서
     확인한 항목은 전체 표기, 그 외는 제1저자 et al. 표기(허위 명단 방지).
     Elsevier name–year 스타일 근사; 조판 시 저널 스타일로 변환. -->
