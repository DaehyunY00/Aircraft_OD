...
## 7.7 Object-level gate 실험 — 가설 반증 (2026-08-04)

§7.5에서 환각 라벨 노이즈가 "RFS 열세를 설명하는 유력 후보"라고 적었다.
검정했고, **지지되지 않았다.**

**설계**: 게이트로 생성물 전량을 검사해 보호 영역 밖 확신 검출이 있는 이미지를
기각하고(uniform 85 / selective 82 / weakness 165장, 총 845개 미라벨 객체 제거),
남은 것만으로 3 arm 재학습. 예산은 915/918/835로 원본보다 8~17% 적다.

**결과 (test mAP50-95, Wilcoxon 클래스 대응)**

| arm | 예산 | all Δ | tail Δ | weak Δ | 최소 p |
|---|---|---|---|---|---|
| uniform_og vs uniform | −85 | +0.0053 | +0.0094 | −0.0090 | 0.385 |
| selective_og vs selective | −82 | −0.0171 | −0.0125 | −0.0213 | 0.110 |
| weakness_og vs weakness | −165 | −0.0008 | −0.0018 | −0.0082 | 0.376 |

**9개 비교 전부 비유의(p = 0.110~0.967).** 어느 arm도 게이트로 유의하게 개선되지
않았다.

**RFS와의 격차는 그대로다.** 게이트 arm 전부 여전히 유의하게 낮다 —
uniform_og Δ=−0.0554, selective_og Δ=−0.0674, weakness_og Δ=−0.0646, 모두 p<0.0001.

**결론**: 환각 라벨 노이즈는 RFS 열세의 원인이 아니다. §7.5에서 세운 가설을
철회한다. 격차의 원인은 다른 데 있으며, 가장 그럴듯한 설명은 이 데이터셋에서
**배경 다양화가 실제 객체 외형의 재노출보다 얻는 게 적다**는 것이다.

**부수 관찰 두 가지.**
1. weakness arm은 데이터의 16.5%를 잃고도 성능 변화가 −0.0008(p=0.967)로 사실상
   0이었다. 제거된 이미지가 기여가 없었다는 뜻이거나, **이 예산대에서 합성 이미지의
   한계 가치가 이미 0에 가깝다**는 뜻이다. 후자라면 예산 확대 실험이 필요하다.
2. 실무적으로 **게이트는 공짜다.** 합성물의 8~17%를 버려도 측정 가능한 손해가 없다.
   라벨 노이즈를 없애고 싶다면 성능 대가 없이 적용할 수 있다 — 다만 이득을 기대해선
   안 된다.

**한계**: 단일 seed이고, 예산이 맞춰지지 않은 비교다(게이트 arm이 8~17% 적음).
다만 방향이 한쪽으로 안전하다 — 게이트가 이겼다면 데이터가 적은데도 이긴 것이라
강한 결론이 됐을 텐데, 지지되지 않았으므로 "노이즈 제거로는 격차가 닫히지 않는다"는
결론만 남는다. 예산을 맞춘 대조군(ungated를 835장으로 축소)은 미실행.

원자료: `gs://military-od-gate/`, `outputs_gate/metrics/per_class_ap.csv`
